...
class LogAction extends CommonAction {
	...
	function index(){
		...
		if($_SESSION['sql']!=$this->getActionName()){
		   ...
		   $_SESSION["log_sql"]=$sql;
		}
		if($_REQUEST['flag']){  //保存状态
		   $_SESSION["log_sql"]=$sql;
		}else{}
        $count = $model->where($_SESSION["log_sql"])->count('id');
		if($count > 0){
		import("@.ORG.Page");
			$p=New Page($count,C('PAGE_LIST_ROWS'));
			$userlist=$model->where($_SESSION["log_sql"])->order("`" . $order . "` " . $sort)->limit($p->firstRow . ',' . $p->listRows)->findAll();
			$page=$p->show();
