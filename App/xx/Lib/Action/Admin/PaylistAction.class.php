...
class PaylistAction extends CommonAction {
	...
	function index(){
		...
		if($_SESSION['sql']!=$this->getActionName()){
		   ...
		   $_SESSION["plist_sql"]=$sql;
		}
		if($_REQUEST['flag']){  //保存状态
		   $_SESSION["plist_sql"]=$sql;
		}
       $count = $model->where($_SESSION["plist_sql"])->count('id');
		if($count > 0){
		import("@.ORG.Page");
			$p=New Page($count,C('PAGE_LIST_ROWS'));
			$userlist=$model->where($_SESSION["plist_sql"])->order("`" . $order . "` " . $sort)->limit($p->firstRow . ',' . $p->listRows)->findAll();
			$page=$p->show();
