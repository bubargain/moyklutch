...
class UserModel extends CommonModel {
	...
	public function getgoods(){
		...
		import("@.ORG.Page");
		$p    = New Page($count,15);
		$list = $goods->field('think_seckill_user.* ,g.title,g.score,g.signupscore,g.failscore')->join('think_seckill_goods AS g ON g.id=think_seckill_user.gid')->where($sql)->order("think_seckill_user.id desc")->limit($p->firstRow . ',' . $p->listRows)->findAll();
