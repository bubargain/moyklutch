...
/**
  ...
  * @author liu21st，zzguo28，who
  +------------------------------------------------------------------------------
 */
class AddscoreAction extends CommonAction {
	...
	function index(){
		...
		$sql="1=1";
		if($_REQUEST['user_id']!=''){
			 $sql=$sql." and user_id=$_REQUEST[user_id]";
		}
		if($_REQUEST['time1']!=''){
		...
		if($_SESSION['sql']!=$this->getActionName()){
		   ...
		   $_SESSION["slist_sql"]=$sql;
		}
		if($_REQUEST['flag']){  //保存状态
		   $_SESSION["slist_sql"]=$sql;
		}
       $count = $model->where($_SESSION["slist_sql"])->count('id');
		if($count > 0){
		import("@.ORG.Page");
			$p=New Page($count,C('PAGE_LIST_ROWS'));
			$userlist=$model->where($_SESSION["slist_sql"])->order("`" . $order . "` " . $sort)->limit($p->firstRow . ',' . $p->listRows)->findAll();
			$page=$p->show();
