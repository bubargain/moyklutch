...
class UserAction extends CommonAction{
	...
	public function delbind(){
		$data['card_id']='';
		$cardid = M('User')->where('id='.(int)$_SESSION['auth'])->getField('card_id');
        M('Card')->where("card_id='".$cardid."'")->setField('status',1);
        M('User')->where('id='.(int)$_SESSION['auth'])->setField('card_id','');
        $this->success('您已解绑成功');
	}
	//提交投诉建议
	...
	public function check(){
		...
		if($type == "email"){//邮箱验证
           ...
           if(empty($_REQUEST['mobile'])){
			...
			import('@.ORG.SMS');
			$sms = new SMS();
			$sms->sendSMS($config['sms_user'],$config['sms_password'],array(intval($_REQUEST['mobile'])),"$verify");
            $d[$field] = $verify;
