...
function get_tradephone($id)
{
	$model   = M("Trade");
	$m=$model->getById($id);
	return $m['phone'];
	}
function get_tradeaddress($id)
{
	$model   = M("Trade");
	$m=$model->getById($id);
	return $m['address'];
...
{
	$model   = M("Ticket");
	$m=$model->where("trade_id=$id and status=1")->count();
	return $m;
	}
...
//展位出租信息
	function get_rendinfo($id){
		...
		if($t&&$t['close_time']>time()){
		  ...
		  return  "<font color='$color'>".date("Y-m-d",$t['close_time']+3600*24)."起生效，当前商家：</font><a target='_blank' href='__APP__/Trade/index/id/$t[trade_id]'>".get_trade($t['trade_id'])."</a>";
		}else{
...
function deldir($dir) {
   ...
   function get_star($pid,$module){
	   $model = M("Comment");
	   $star  = $model->where("pid=$pid and module='".$module."'")->avg('star');
		if($star){
			 return (int)$star;
			 }else{
			 return 1;
	...
	//评论统计
   function get_comment($pid,$module){
	   $model = M("Comment");
	   $count = $model->where("pid=$pid and module='".$module."'")->count();
	   return $count;
	   }

	//得到注册所送积分
	function get_regscore(){
		$model = M("Config");
		$score = $model->find();
		return $score['reg_if'];
		}
	...
	//截取字符串
	function t_substr($str,$a=0,$b=10){
		return mb_substr($str,$a,$b,'utf-8');
	}

	//检查积分
	function ckscore(){
		return M('User')->where('id='.$_SESSION['auth'])->getField('score');
	}

	//成功秒杀用户
	function get_succeeduser($gid){
		$user = M()->query("SELECT u.account FROM think_user AS u LEFT JOIN think_seckill_user AS a ON u.id=a.uid WHERE a.gid='{$gid}' AND a.sectype<>0");
		foreach($user as $v){
			$u .= $v['account'].'<br>';
		}
		return $u;
    ...
    function getMYinfo($userid,$field)
    {
        $userid = (int)$userid;
        ...
        {
            case 'score':
               $abc = M('User')->where('id='.$userid)->getField('score');
               return (int)$abc;
                break;
            case 'msgnum':
                import('@.ORG.Uc');
                $Uc     = new Uc();
                $val    = $Uc->get_msgnum($_SESSION['uc_uid'],0);
                return (int)$val;
                break;
            default:
        }
