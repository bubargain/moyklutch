...
/************************************************************************************************************************/		
		...
		if(!empty($_COOKIE['Yhj_auth'])) {
			require_once 'uc_client/client.php';
			list($_SGLOBAL['uc_user'][0], $_SGLOBAL['uc_user'][1]) = explode("\t", uc_authcode($_COOKIE['Yhj_auth'], 'DECODE'));
			//RBAC
			FLEA::loadClass('FLEA_Rbac_UsersManager');
			$_user =& FLEA::getSingleton('Model_User');
			FLEA::loadClass('FLEA_Rbac');
			$_rbac = & FLEA::getSingleton('FLEA_Rbac');
			//建立SESSION
			$_SGLOBAL['login_user'] = $_user->findByUserId($_SGLOBAL['uc_user'][0]);
			$_SGLOBAL['login_user']['RBAC_ROLES'] = array();
			if(isset($_SGLOBAL['login_user']['roles']))
			{
				foreach( $_SGLOBAL['login_user']['roles'] as  $key => $roles)
				{
					$_SGLOBAL['login_user']['RBAC_ROLES'][] = $roles['rolename'];
				}
			}
			$_rbac->setUser($_SGLOBAL['login_user']);
		}
		else
		{
			FLEA::loadClass('FLEA_Rbac');
