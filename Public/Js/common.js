...
function imports(){
		 location.href  = URL+"/imports/";
}
function sendmail(){
		 location.href  = URL+"/sendmail/";
}
...
function hidden_more (id){
	var o=document.getElementById(id);
	o.style.display='none';
}
//修改备注等ajax方法
function aupdate(m,id,col){ 
   var val=document.getElementById(col+id).value;
   var param=new Array();
   $.ajax({
	url:encodeURI(URL+'m='+m+'&a=aupdate&id='+id+'&'+col+'='+val),
       type:'post',         //数据发送方式 
...
function resetPwd(){
var pass = document.getElementById('resetPwd').value;
var id = document.getElementById('id').value;
	...
	{   
	   ...
	   var param=new Array();
	   $.ajax({
		url:encodeURI(APP+'?g=Admin&m=User&a=resetPwd&id='+id+'&password='+pass),
		   type:'post',         //数据发送方式 
		   dataType:'json',     //接受数据格式 
		   data:param,         //要传递的数据 
		   success:update_page2  //回传函数(这里是函数名) 
		 });
	 }
}
...
function recycle(id){
	if (window.confirm('确实要恢复选择项吗？'))
	{
	   var param=new Array();
	   $.ajax({
		url:encodeURI(URL+'/recycle/id/'+id),
		   type:'post',         //数据发送方式 
		   dataType:'json',     //接受数据格式 
		   data:param,         //要传递的数据 
		   success:del_page3 //回传函数(这里是函数名) 
		 });
...
function forbidden(id){
	if (window.confirm('确实要禁用选择项吗？'))
	{
	   var param=new Array();
	   $.ajax({
		url:encodeURI(URL+'/foreverdelete/id/'+id),
		   type:'post',         //数据发送方式 
		   dataType:'json',     //接受数据格式 
		   data:param,         //要传递的数据 
		   success:del_page  //回传函数(这里是函数名) 
		 });
	   document.getElementById("row"+id).style.display='none';
	  }
	 }
function dealerror(id){
	if (window.confirm('确实已处理吗？'))
	{
	   var param=new Array();
	   $.ajax({
		url:encodeURI(URL+'/foreverdelete/id/'+id),
...
function adel(id){
	if (window.confirm('确实要删除选择项吗？'))
	{
	   var param=new Array();
	   $.ajax({
		url:encodeURI(URL+'/delete/id/'+id),
		   type:'post',         //数据发送方式 
		   dataType:'json',     //接受数据格式 
		   data:param,         //要传递的数据 
		   success:del_page  //回传函数(这里是函数名) 
...
function aupdate_money(m,id,col){ 
   var val=document.getElementById(col+id).value;
...
function showuser(id,my)  //授权用户 
{
 window.open(URL+'/touser/id/'+id,'','height=500,width=430');   
}

function showexh(id)  //显示展位租赁信息
{
 window.open(URL+'/showexh/id/'+id,'','height=700,width=900');   
}
function applyexh(id)  //显示展位信息
{
 location.href =APP+'/Admin/Myexh/applyexh/id/'+id;   
}
function apply(id)  //申请展位
{
 location.href = APP+'/Admin/Myexh/apply/id/'+id;   
}

function showcount(m,action,id)  //统计 
{
 window.open(APP+'/Admin/'+m+'/showcount/id/'+id+'/type/'+action,'','height=600,width=800');   
} 
function showscore(m,id)  //积分
{
 window.open(APP+'/Admin/'+m+'/showscore/id/'+id,'','height=600,width=800');   
} 
function showmoney(m,id)  //积分
{
 window.open(APP+'/Admin/'+m+'/showmoney/id/'+id,'','height=600,width=800');   
} 
function showgetmoney(m,id)  //积分换算
{
 window.open(APP+'/Admin/'+m+'/showgetmoney/id/'+id,'','height=600,width=800');   
}
function showticket(id)  //商家优惠券
{
 window.open(APP+'/Admin/Ticket/index/trade_id/'+id,'','height=600,width=800');   
}

function showten(id)  //地点租赁
{
 window.open(APP+'/Admin/Tenancy/index/p_id/'+id,'','height=600,width=800');   
}
function showprinten(id)  //打印展位租赁
{
 window.open(APP+'/Admin/Location/showten/id/'+id,'','height=600,width=800');   
}

function delten(pid,bt){
 if (window.confirm('确实要删除吗？'))
	{
		location.href = APP+'/Admin/Location/delten/pid/'+pid+'/bt/'+bt;
	}
}

function formsubmit(){
location.href = URL+'/index/submit/exp';
	}
function tomorrow(){
location.href = URL+'/index/submit/tomorrow';
	}
function pay_add(){
location.href = APP+'/Admin/Pay/index';
	}