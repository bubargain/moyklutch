...
* @version 3.5.2 (2010-07-29) txhyq.com 修改版
*******************************************************************************/
...
KE.setting = {
	...
	statusbarHeight : 11,
	items : [
		'source', '|', 'new', 'open', 'save', 'preview', '|', 'template', '|', 'undo', 'redo', '|',
		'cut', 'copy', 'paste', '|', 'image', 'specialchar', 'hr','|', 'selectall', 'removeformat', '|',
		'showblocks', '|', 'fontsize', 'lineheight', 'fanse', '|', 'justifyleft', 'justifycenter', 'justifyright'
	],
	colorTable : [
	...
	htmlTags : {
		...
		'p,ol,ul,li,blockquote,h1,h2,h3,h4,h5,h6' : [
			...
			'.font-weight', '.font-style', '.text-decoration', '.vertical-align', '.text-indent', '.margin-left', '.line-height',
...
KE.queryCommandValue = function(doc, cmd) {
	...
	if (cmd === 'fontname') {
		...
		var keSel = new KE.selection(doc);
		var el = KE.getCommonAncestor(keSel, 'span');
		if (el) val = KE.getComputedStyle(el, 'line-height');
		if (val === '') {
			el = KE.getCommonAncestor(keSel, 'p');
			if (el) val = KE.getComputedStyle(el, 'line-height');
		}
...
KE.util = {
	...
	resize : function(id, width, height, isCheck, isResizeWidth) {
		...
		if (diff >= 0) {
			...
			g.previewDiv.style.height = diff + 'px';
		}
	},
	...
	getFullHtml : function(id, body) {
		...
		html += 'body {\n';
		html += '	margin: 0;\n';
		html += '	padding: 40px 38px 40px 38px;\n';
		html += '	text-align:left;\n';
		html += '	background:url(/imgs/biao.gif) right top no-repeat;\n';
		html += '	color: #000000;\n';
		html += '	font: 24px/24px "宋体";\n';
		html += '}\n';
		html += 'p {\n';
		html += '	margin:0;\n';
		html += '	padding:0;\n';
		...
		html += '	color: #fff;\n';
		html += '	background: #000;\n';
		html += '	padding: 3px 0px;\n';
		html += '}\n';
		html += '.ke-show-blocks p {\n';
		html += '	border:1px dotted #CCC;\n';
		html += '	background-image:url(' + KE.g[id].skinsPath + 'common/p.gif);\n';
		html += '	background-position:right top;\n';
		html += '	background-repeat:no-repeat;\n';
		html += '}\n';
		html += '.ke-show-blocks div {\n';
		html += '	border:1px dotted #CCC;\n';
		html += '	background-image:url(' + KE.g[id].skinsPath + 'common/div.gif);\n';
		html += '	background-position:right top;\n';
		html += '	background-repeat:no-repeat;\n';
		html += '}\n';
		html += '.ke-show-blocks ul {\n';
		html += '	border:1px dotted #CCC;\n';
		html += '	background-image:url(' + KE.g[id].skinsPath + 'common/ul.gif);\n';
		html += '	background-position:right top;\n';
		html += '	background-repeat:no-repeat;\n';
		html += '}\n';
		html += '.ke-show-blocks ol {\n';
		html += '	border:1px dotted #CCC;\n';
		html += '	background-image:url(' + KE.g[id].skinsPath + 'common/ol.gif);\n';
		html += '	background-position:right top;\n';
		html += '	background-repeat:no-repeat;\n';
		html += '}\n';
		html += '.ke-show-blocks dl {\n';
		html += '	border:1px dotted #CCC;\n';
		html += '	background-image:url(' + KE.g[id].skinsPath + 'common/dl.gif);\n';
		html += '	background-position:right top;\n';
		html += '	background-repeat:no-repeat;\n';
		html += '}\n';
		html += '</style>\n';
...
KE.create = function(id, mode) {
	...
	/* 20100803 */
	var previewCell = textareaTable.rows[0].insertCell(1);
	...
	/* 20100803 */
	var previewDiv = KE.$$('div');
	previewDiv.className = 'ke-preview-div';
	KE.g[id].container = container;
	KE.g[id].iframe = iframe;
	KE.g[id].newTextarea = newTextarea;
	KE.g[id].previewDiv = previewDiv;
...
KE.plugin['fontsize'] = {
	click : function(id) {
		...
		var font = KE.queryCommandValue(KE.g[id].iframeDoc, 'fontname');
		var height = KE.queryCommandValue(KE.g[id].iframeDoc, 'lineheight');
		if(height==''){
			height='24px';
		}
		if(size==''){
		...
		height = parseInt(height, 10)-parseInt(size, 10);
		font = font.toLowerCase() == 'courier new' ? 'enfont' : 'cnfont';
		if(font==='cnfont'){
			if(size=='24px') font = 'cnfont1';
			if(size=='48px') font = 'cnfont2';
			if(size=='72px') font = 'cnfont3';
		}else{
			if(size=='8px') font = 'enfont1';
			if(size=='16px') font = 'enfont2';
			if(size=='24px') font = 'enfont3';
		}
		var menu = new KE.menu({
			id : id,
			cmd : cmd,
		...
		KE.each(fontSize, function(key, val) {
			var html = '<span class="ke-reset">' + val + '</span>';
			menu.add(
				html,
				function() {
					KE.plugin[cmd].exec(id, key, height);
				},
				{
					checked : (font === key)
				}
	...
	exec : function(id, key, height) {
		var keSel = KE.g[id].keSel,
			keRange = KE.g[id].keRange,
			sc = keRange.startNode,
			ec = keRange.endNode,
			doc = KE.g[id].iframeDoc;
		//var arr = key.split(',');
		var fontName = key;
		if(fontName==='cnfont1') var fontSize = 24;
		if(fontName==='cnfont2') var fontSize = 48;
		if(fontName==='cnfont3') var fontSize = 72;
		if(fontName==='enfont1') var fontSize = 8;
		if(fontName==='enfont2') var fontSize = 16;
		if(fontName==='enfont3') var fontSize = 24;

		//var fontSize = arr[1];
		
		var lineHeight = fontSize + height + 'px';
		if(typeof(sc.tagName)!='undefined' && sc.tagName.toLowerCase() == 'p'){
			var node = sc;
		}else{
			var node = sc.parentNode;
		}
		...
		while (node) {
			...
			if (node.innerHTML.toLowerCase().indexOf('<img') >= 0) {
				alert('图片无法设置字体大小');
			}else{
				if (fontName === 'cnfont1' || fontName === 'cnfont2' || fontName === 'cnfont3' || !/[\u4e00-\u9fa5]/.test(node.innerHTML)) {
					KE.removeClass(node, 'cnfont1');
					KE.removeClass(node, 'cnfont2');
					KE.removeClass(node, 'cnfont3');
					KE.removeClass(node, 'enfont1');
					KE.removeClass(node, 'enfont2');
					KE.removeClass(node, 'enfont3');
					KE.addClass(node, fontName);
					node.style.lineHeight = lineHeight;
				}else{
					alert('中文内容无法使用英文字体');
				}
			}
			if (node === ec.parentNode) {
				break;
			}
			node = next;
		}
...
KE.plugin['lineheight'] = {
	click : function(id) {
		...
		var cmd = 'lineheight';
		KE.util.selection(id);
		var size = KE.queryCommandValue(KE.g[id].iframeDoc, 'fontsize') || '24px';
		var height = KE.queryCommandValue(KE.g[id].iframeDoc, 'lineheight');
		var menu = new KE.menu({
			id : id,
			cmd : cmd,
		...
		KE.each(lineHeight, function(key, val) {
			var px = (parseInt(size, 10) + parseInt(key, 10)) + 'px';
			var html = '<span class="ke-reset">' + val + '</span>';
			menu.add(
				html,
				function() {
					KE.plugin[cmd].exec(id, px);
				},
				{
					checked : (height === px)
				}
	...
	exec : function(id, lineHeight) {
		...
		while (node) {
			var next = node.nextSibling;
			if (node.className.indexOf('fanse') >= 0) {
				alert('反色区域无法调整行高');
			}else{
				if (node.innerHTML.toLowerCase().indexOf('<img') >= 0) {
					alert('图片无法调整行高，请右击图片设置属性');
				}else{
					node.style.lineHeight = lineHeight;
					if (node === ec.parentNode) {
						break;
...
KE.plugin['hr'] = {
	click : function(id) {
		KE.util.selection(id);
		KE.util.insertHtml(id, '<p class="enfont1">----------------------------------------------------------------</p>');
		KE.util.focus(id);
	}
};
...
KE.plugin['removeformat'] = {
	click : function(id) {
		KE.util.selection(id);
		KE.util.execCommand(id, 'removeformat', null);
		KE.util.selection(id);
		var keSel = KE.g[id].keSel,
			keRange = KE.g[id].keRange;
		if (keRange.collapsed()) {
			var el = KE.getCommonAncestor(keSel, 'p');
			if (!el) {
				return;
			}
			el.style.cssText = '';
			...
			cmd.remove(tags);
			var el = KE.g[id].iframeDoc.body.firstChild;
			if (KE.util.inArray(el.nodeName.toLowerCase(), ['p', 'div'])) {
				el.style.cssText = '';
				el.className = '';
			}
		}
...
KE.plugin['template'] = {
	...
	insert : function(id, file) {
		var self = this;
		jQuery.get(KE.g[id].pluginsPath + 'template/' + file, function(data) {
...
KE.plugin['image'] = {
	...
	insert : function(id, url, width, height) {
		var html = '<p><img src="' + url + '" kesrc="' + url + '" ';
...
KE.plugin['editimage'] = {
	...
	check : function(id) {
		...
		var tmarginBox = KE.$('tmargin', dialogDoc).value;
		var bmarginBox = KE.$('bmargin', dialogDoc).value;
		var lmarginBox = KE.$('lmargin', dialogDoc).value;
		var rmarginBox = KE.$('rmargin', dialogDoc).value;
	...
	exec : function(id) {
		var self = this;
		var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
		var tmargin = KE.$('tmargin', dialogDoc).value;
		var bmargin = KE.$('bmargin', dialogDoc).value;
		var lmargin = KE.$('lmargin', dialogDoc).value;
		var rmargin = KE.$('rmargin', dialogDoc).value;
		if (!tmargin.match(/^\d*$/)) {
			alert('上间距必须为数字。');
			KE.$('tmargin', dialogDoc).focus();
			return;
		}
		if (!bmargin.match(/^\d*$/)) {
			alert('下间距必须为数字。');
			KE.$('bmargin', dialogDoc).focus();
			return;
		}		if (!tmargin.match(/^\d*$/)) {
			alert('左间距必须为数字。');
			KE.$('lmargin', dialogDoc).focus();
			return;
		}
		if (!bmargin.match(/^\d*$/)) {
			alert('右间距必须为数字。');
			KE.$('rmargin', dialogDoc).focus();
			return;
		}
		var imgNode = self.getSelectedNode(id);
		if (imgNode) {
			imgNode.style.marginLeft = lmargin + 'px';
			imgNode.style.marginRight = rmargin + 'px';
			imgNode.style.marginTop = tmargin + 'px';
			imgNode.style.marginBottom = bmargin + 'px';
		}
		this.dialog.hide();
		KE.util.focus(id);
	}
...
KE.plugin['open'] = {
	...
	exec : function(id) {
		var self = this;
		var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
		var urlBox = KE.$('htmlFile', dialogDoc);
		if (!urlBox.value.match(/\.(html|htm)$/i)) {
			alert('只能选择HTML文件。');
			urlBox.focus();
			return false;
		}
		...
		var onloadFunc = function() {
			...
			KE.html(id, uploadDoc.body.innerHTML);
			self.dialog.hide();
		};
...
KE.plugin['save'] = {
	click : function(id) {
		var form = KE.$('editorForm');
		var action = form.action;
		var saveIframe = KE.$('saveIframe');
		var onloadFunc = function() {
			KE.event.remove(saveIframe, 'load', onloadFunc);
			form.action = action;
			form.target = '';
		};
		KE.event.add(saveIframe, 'load', onloadFunc);
		KE.$('htmlData').value = KE.util.getFullHtml(id, KE.html(id));
		form.action = KE.scriptPath + 'php/save.php?id='+id;
		form.target = 'saveIframe';
		KE.$('editorForm').submit();
	}
};

KE.plugin['showblocks'] = {
	click : function(id) {
		var body = KE.g[id].iframeDoc.body;
		if (/ke-show-blocks/i.test(body.className)) {
			KE.removeClass(body, 'ke-show-blocks');
		} else {
			KE.addClass(body, 'ke-show-blocks');
		}
	}
};
...
KE.plugin['fanse'] = {
	click : function(id) {
		var keSel = new KE.selection(KE.g[id].iframeDoc),
			keRange = keSel.keRange,
			sc = keRange.startNode,
			ec = keRange.endNode,
			doc = KE.g[id].iframeDoc;
		var size = KE.queryCommandValue(KE.g[id].iframeDoc, 'fontsize') || '24px',
			lineHeight = (parseInt(size, 10) + 6) + 'px';
		if(typeof(sc.tagName)!='undefined' && sc.tagName.toLowerCase() == 'p'){
			var node = sc;
		}else{
			var node = sc.parentNode;
		...
		while (node) {
			...
			if (node.className.indexOf('fanse') >= 0) {
				...
				KE.removeClass(node, 'fanse');
			} else {
				if(node.innerHTML.toLowerCase().indexOf('<img') >= 0){
					alert('图片无法反色');
				}else{
					node.style.lineHeight = size;
					KE.addClass(node, 'fanse');
				}
			}
			if (node === ec.parentNode) {
				break;
...
KE.plugin['preview'] = {
	click : function(id) {
		jQuery.post(KE.scriptPath + 'php/preview.php?id='+id, {htmlData : KE.html(id)}, function(data) {
			var div = KE.g[id].previewDiv,
				divHeight = div.offsetHeight;
			div.style.height = 'auto';
			div.innerHTML = '<img src="' + KE.scriptPath + 'image/prev/' + data + '" />';
			setTimeout(function() {
				if (div.offsetHeight > divHeight) {
					var height = parseInt(KE.g[id].height) + div.offsetHeight - divHeight;
					KE.util.resize(id, null, height + 'px', false, false);
				}
			}, 0);
		});
	}
